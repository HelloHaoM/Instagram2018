...
//  Main controller of Home page, including Bluetooth function, sorting function,
//  and show all users' posts
...
class HomeController: HomePostCellViewController, CLLocationManagerDelegate {
    ...
    override func viewDidLoad() {
        ...
        collectionView?.register(
            HomePostCell.self, forCellWithReuseIdentifier: HomePostCell.cellId)
        collectionView?.backgroundView = HomeEmptyStateView()
        collectionView?.backgroundView?.alpha = 0
        
        NotificationCenter.default.addObserver(
            self, selector: #selector(handleRefresh),
            name: NSNotification.Name.updateHomeFeed, object: nil)
    ...
    private func configureAlertController() {
        ...
        let sortByTimeAction = UIAlertAction(title: "Sort by time",
                                             style: .default) { (_) in
            do {
                self.posts.sort(by: { (p1, p2) -> Bool in
                    return p1.creationDate.compare(p2.creationDate) ==
                        .orderedDescending
                })
        ...
        let sortByLocationAction = UIAlertAction(title: "Sort by location",
                                                 style: .default) { (_) in
            do {
                ...
                if CLLocationManager.locationServicesEnabled() {
                    ...
                    self.posts.sort(by: { (p1, p2) -> Bool in
                        let coordinate1 = CLLocation(latitude: p1.location[0],
                                                     longitude: p1.location[1])
                        let coordinate2 = CLLocation(latitude: p2.location[0],
                                                     longitude: p2.location[1])
                        return self.distance(to: coordinate1) <
                            self.distance(to: coordinate2)
                    })
                    self.collectionView?.reloadData()
                }
    ...
    private func configureNavigationBar() {
        navigationItem.titleView = UIImageView(
            image: #imageLiteral(resourceName: "logo").withRenderingMode(.alwaysOriginal))
        navigationItem.leftBarButtonItem = UIBarButtonItem(
            image: #imageLiteral(resourceName: "camera3").withRenderingMode(.alwaysOriginal), style: .plain,
            target: self, action: #selector(handleCamera))
        
        let inRangeButton = UIBarButtonItem(
            image: #imageLiteral(resourceName: "people_near").withRenderingMode(.alwaysOriginal), style: .plain,
            target: self, action: #selector(handleInRange))
        let sortButton = UIBarButtonItem(
            image: #imageLiteral(resourceName: "inbox").withRenderingMode(.alwaysOriginal), style: .plain,
            target: self, action: #selector(handleSort))
    ...
    private func fetchPostsForCurrentUser() {
        ...
        Database.database().fetchAllPosts(withUID: currentLoggedInUserId,
                                          completion: { (posts) in
    ...
    private func fetchFollowingUserPosts() {
        ...
        Database.database().reference().child("following")
            .child(currentLoggedInUserId).observeSingleEvent(of: .value,
                                                             with: { (snapshot) in
            guard let userIdsDictionary = snapshot.value as? [String: Any] else { return }
            
            userIdsDictionary.forEach({ (uid, value) in
                
                Database.database().fetchAllPosts(withUID: uid,
                                                  completion: { (posts) in
    ...
    override func showEmptyStateViewIfNeeded() {
        guard let currentLoggedInUserId = Auth.auth().currentUser?.uid else { return }
        //get the number of users that current user is following
        Database.database().numberOfFollowingForUser(
        withUID: currentLoggedInUserId) { (followingCount) in
            Database.database().numberOfPostsForUser(
                withUID: currentLoggedInUserId, completion: { (postCount) in
    ...
    func locationManager(_ manager: CLLocationManager,
        ...
        currentLocation = CLLocation(latitude: locValue.latitude,
...
extension HomeController: UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(
        ...
        let dummyCell = HomePostCell(
            frame: CGRect(x: 0, y: 0, width: view.frame.width, height: 1000))
